...
const isoCurrencyToSymbol: Map<ISOCurrency, string> = new Map();
const symbolToIsoCurrency: Map<string, ISOCurrency[]> = new Map();

for (let [key, value] of Object.entries(currencyList)) {
	const symbolString = value[4] as string;
	const currency = key as ISOCurrency;
	isoCurrencyToSymbol.set(currency, symbolString);

	let array = symbolToIsoCurrency.get(symbolString);
	if (array == null) {
		array = [];
		symbolToIsoCurrency.set(symbolString, array);
	}
	array.push(currency);
}

export const ISOCurrencyToSymbol: ReadonlyMap<ISOCurrency, string> = isoCurrencyToSymbol;
export const SymbolToISOCurrency: ReadonlyMap<string, readonly ISOCurrency[]> = symbolToIsoCurrency;
