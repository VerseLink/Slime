...
describe('JWTManager', () => {
	...
	it('should issue a certificate', async () => {
		const jwtManager = new JWTManager(env.JWT_KEY_KV_STORE, { defaultExpirySeconds: 3600 });
	...
	it('should throw an error when signing with an invalid certificateId', async () => {
		...
		await expect(jwtManager.sign(payload, 'nonexistent-cert-id')).rejects.toThrow('Certificate not found');
	});
	...
	it('should correctly save and retrieve certificates in KV store', async () => {
		...
		expect(retrievedCerts.some((x) => x.id === cert.id)).toBe(true);
	});
	...
	it('should not verify tokens signed with a revoke certificate', async () => {
		...
		const token2 = await jwtManager.sign(payload, cert2.id);
